...
if __name__ == '__main__':
    ...
    for item_idx in tqdm(range(len(dataset))):
        ...
        stitch_tags = cloths_info['stitch_tags'].unsqueeze(0)
        ...
        input_data = torch.concat([gt_seq, discrete_quantization.unsqueeze(0)], dim=-1)
        input_data = einops.rearrange(input_data, 'b p e c -> b (p e) c')
        input_data = input_data.to(weight_dtype).to(device)
